...
public class PotionHelper {
    ...
    public static TranslationTextComponent getItemTranslation(ItemStack stack) {
        String prefix =  getItemMetadata(stack) == 0 ? "error" : getPotionPrefix(getItemMetadata(stack));
        return new TranslationTextComponent(stack.getItem().getTranslationKey(), translationString("misc.beta_brewing_system.potion." + prefix));
    }

    public static TranslationTextComponent getFluidTranslation(FluidStack stack) {
        String prefix =  getFluidData(stack) == 0 ? "error" : getPotionPrefix(getFluidData(stack));
        return new TranslationTextComponent(stack.getTranslationKey(), translationString("misc.beta_brewing_system.potion." + prefix));
    }

    private static int getFluidData(FluidStack stack) {
        return stack.getOrCreateTag().getInt("fluidData");
    }
