...
public class CauldronBlockEntityRenderer implements BlockEntityRenderer<CauldronBlockEntity> {
    ...
    public void render(CauldronBlockEntity blockEntityIn, float partialTicks, PoseStack matrixStack, MultiBufferSource buffers, int light, int overlay) {
        if (blockEntityIn.getAmount() > 0) RenderHelper.renderFluidTankFluid(blockEntityIn, matrixStack, buffers,
            (blockEntity, poseStack, fluidStack, bufferIn, info) -> {
                int color;
                if (fluidStack.getFluid() == ModFluids.POTION.getObject())
                    color = blockEntityIn.getPotionFluidColor();
                else if (fluidStack.getFluid() == Fluids.WATER)
                    color = FluidHelper.getFluidColorWithBiome(fluidStack, blockEntity);
                else color = -1;
                return color;
            }, light, CubeRenderInfo.create(Direction.UP, new Vector3f(1, 1, 1), new Vector3f(0, ((1 - (blockEntityIn.getAmount() / (float) blockEntityIn.getCapacity())) * 0.7f) + 0.05f, 0))
        );
    }
}
